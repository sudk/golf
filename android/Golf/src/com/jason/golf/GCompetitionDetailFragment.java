...
public class GCompetitionDetailFragment extends Fragment implements
		OnClickListener {
	...
	private TextView mName, mStartDate, mEndDate, mAgent, mCourt, mPlan, mDesc,
			mFee, mInclude, mExclude;

	private LinearLayout mCompetitionImages;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mCompetitionImages = (LinearLayout) v
				.findViewById(R.id.competition_images);
	...
	private void queryCompetition() {
		// TODO Auto-generated method stub
		if (TextUtils.isEmpty(_competitionId))
			return;
					...
					public void sucessData(String res) {
						...
						try {
							...
							if (_competition.init(data)) {
								...
								mFee.setText(String.format("￥%.2f %s",
										(float) fee / 100, GCompetition.GetFeeTypeDes(_competition.getFeeType())));
								...
								ArrayList<String> imgUrl = _competition.getImgs();
								...
								for (int i = 0; i < length; i++) {
									...
									img.setPadding(4, 2, 4, 2);
									mCompetitionImages.addView(img);

									if (imgUrl.size() > i)
										_fb.display(img, imgUrl.get(i));
								}
								
								mCompetitionImages.setOnClickListener(new View.OnClickListener() {
									
									@Override
									public void onClick(View v) {
										// TODO Auto-generated method stub
										
										if(_competition.getImgs().size() > 0){
											
											Bundle params = new Bundle();
											
											String[] urls = new String[_competition.getImgs().size()];
											for(int i=0,length=_competition.getImgs().size(); i<length;i++){
												urls[i] = _competition.getImgs().get(i);
											}
											
											params.putStringArray(GCompetitionImagesFragment.KEY_IMG_URL, urls);
											
											Fragment imgFragment = GCompetitionImagesFragment.Instance();
							...
							System.out.println(_competition.toString());
	...
	public void onClick(View v) {
		...
		case R.id.competition_sign_up:
			...
			GolfAppliaction app = (GolfAppliaction) getActivity()
					.getApplication();
			if (app.getAccount().isLogin()) {

				// CompetitionApplyingDialog dialog = new
				// CompetitionApplyingDialog(getActivity(), _competition);
				// dialog.show();

				Intent it = new Intent(getActivity(), GOrderActivity.class);
				Bundle params = new Bundle();
				params.putInt(GOrderActivity.FRAGMENT_MARK,
						GOrderActivity.FRAGMENT_MARK_GENERATE_APPLY);
				params.putString(GOrderApplyFragment.key_agent_id,
						_competition.getAgentId());
				params.putString(GOrderApplyFragment.key_pay_type,
						_competition.getFeeType());
				params.putString(GOrderApplyFragment.key_relation_id,
						_competition.getId());
				params.putString(GOrderApplyFragment.key_relation_name,
						_competition.getName());
				params.putString(GOrderApplyFragment.key_tee_time,
						_competition.getStartDate());
				params.putString(GOrderApplyFragment.key_agent_name,
						_competition.getAgentName());
				params.putString(GOrderApplyFragment.key_type,
						GOrderApplyFragment.TYPE);
				params.putInt(GOrderApplyFragment.key_unitprice,
						_competition.getFee());
				...
				Intent itLogin = new Intent(getActivity(),
						GAccountActivity.class);
				itLogin.putExtra(GAccountActivity.FRAGMENT,
						GAccountActivity.LOGIN);
