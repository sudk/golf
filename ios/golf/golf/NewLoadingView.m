...
+ (NewLoadingView *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	NewLoadingView *hud = [[NewLoadingView  alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return [hud autorelease];
}
+(NSString *)MD5With:(NSString *)mdString{
    const char *cStr = (const char *)[mdString UTF8String];  
...
//    BOOL returnR;
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[NewLoadingView  class]]) {
			viewToRemove = v;
            NewLoadingView  *HUD = (NewLoadingView  *)viewToRemove;
...
//        CGSize dims = [self.labelText sizeWithFont:[UIFont fontWithName:@"Arial" size:12]];
        ...
        NSLog(@"label.text======in newloading class======%@",label.text);
//        label.lineBreakMode = UILineBreakModeWordWrap;
        ...
        CGRect lFrame = CGRectMake(floorf((frame.size.width - lWidth) / 2) + xOffset,
                                   ...
                                   lWidth, lHeight);
        label.frame = lFrame;
        NSLog(@"lFrame=====width===%f,height======%f,x====%f,y======%f",lFrame.size.width,lFrame.size.height,lFrame.origin.x,lFrame.origin.y);
