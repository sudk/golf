...
public class UpmpService{
    ...
    public static String buildReq(Map<String, String> req, Map<String, String> resp) {
        ...
        // 签名结果与签名方式加入请求提交参数组中
        req.put(UpmpConfig.SIGNATURE, signature);
        req.put(UpmpConfig.SIGN_METHOD, UpmpConfig.SIGN_TYPE);
    ...
    private static boolean verifyResponse(String respString, Map<String, String> resp) {
        if (respString != null && !"".equals(respString)) {
            ...
            if (signIsValid) {
                String respCode = para.get(UpmpConfig.RESPONSE_CODE);
                if (UpmpConfig.RESPONSE_CODE_SUCCESS.equals(respCode)) {
                    resp.putAll(para);
                    return true;
                }
                resp.put(UpmpConfig.RESPONSE_CODE, respCode);
                resp.put(UpmpConfig.RESPONSE_MSG, para.get(UpmpConfig.RESPONSE_MSG));
            } else{
                return false;
            }
